...
def run_pipeline(alloy: AlloyInput, properties: Optional[ElementPropertyTable] = None,
    ...
    unresolved = []
    if normalized["status"] != "AVAILABLE": unresolved.append("normalized_input.atomic_fractions")
    unresolved.extend(f"composition_descriptors.{k}" for k, v in descriptors.items() if v["status"] != "AVAILABLE")
