...
| ISS-005 | Small and imbalanced independent target classes. | Limits stable training, calibration, subgroup evaluation, and performance claims. | P1 | OPEN | Grouped Split Design V1 confirms only four negative-supporting strict groups per binary target; M2 retains just two TRIP negatives/two groups but all six TWIP negatives/four groups. Use only `VALID_LIMITED` target-specific designs and expand diverse independent negatives without fabrication or resampling. |
| ISS-006 | Recovery V12 replacement-aware strict role counting identifies 51 independent experimental ML conditions, with only 32 TRIP-labelled, 30 TWIP-labelled, and 27 jointly labelled. | Effective sample size remains small despite 192 observation rows. | P1 | OPEN | Grouped Split Design V1 preserves the roster and identifies three M2-compatible T1 holdouts and nine M2-compatible T2 partitions; these establish feasibility only, not modelling adequacy. Continue targeted source review without fabricating support. |
...
| ISS-014 | No final target or final ML-ready dataset exists. | Model comparison/publication claims would be premature. | P1 | OPEN | Feature Schema V1 and Grouped Split Design V1 now freeze predictor eligibility and limited target-specific partitions, but target selection, evidence gaps, M2 representation construction, provenance re-audit, and targeted expansion remain open. T3A is not currently validatable; no model may yet be trained. |
...
| 3 | Hierarchical experimental identity | IMPLEMENTED WITH CONSERVATIVE FALLBACKS: 51-condition replacement-aware roster; 19 missing strict/study/material identities stay inside paper fallback groups and all physical batches remain unknown. |
...
| 9 | Grouped/external validation | SPLIT DESIGN V1 COMPLETE: deterministic T1/T2 candidate feasibility and manifests exist; no model validation or performance result has occurred. |
...
|---|---|
| Current stage | Global Dataset QC V12, Feature Schema V1, and Grouped Split Design V1 are complete. Target-specific limited train/validation candidates and a chemistry-source policy are frozen enough for source-preserving M2 matrix construction with gates; no ML training, feature transformation, or performance evaluation has occurred. |
...
| Current grouped split design | `data/splits/split_candidates_v1.csv` and `split_manifest_v1.csv` use `Leakage_Group_Strict` with paper fallback. M2-compatible valid-limited candidates: T1=3 deterministic grouped holdouts; T2=9 partitions across GroupKFold k=2/k=4 and deterministic holdouts. Retained holdouts have zero exact-source alloy-family overlap; T2 GroupKFold is a G3 paper design and crosses the exact `Fe50Mn30Co10Cr10` family. LOPO is infeasible for both; T3A four-class is not currently validatable. |
...
| Current target status | Under review. Experimental usable conditions are TRIP 32 (27/5), TWIP 30 (24/6), and joint 27 (00=1, 10=5, 01=4, 11=17); 24 conditions have at least one unresolved target component. Negative support spans four strict groups per binary target, but raw M2 completeness leaves only two TRIP negatives/two groups. P017 native labels remain experimental-target-ineligible. |
| Major unresolved issue | Twenty-four experimental conditions have incomplete targets; P018/P019 sources are unavailable; independent negative families, measured chemistry, initial microstructure, experimental SFE, and DeltaG remain sparse. Paper/material dependence, an exact source-text alloy family spanning four papers, potentially equivalent differently written families, 19 conservative paper-fallback conditions, unknown physical batches, M2's two-negative T1 subset, and the joint 00 singleton prevent strong or confirmatory ML claims. |
| Next action | Predeclare `T1_GH_STRICT_01` and the complete T2 GroupKFold-k=2 design, then construct and audit a source-preserving untransformed M2 condition table using measured-bulk-first/nominal-fallback plus `Composition_Source`. Continue negative-family and target/provenance recovery. Do not train, impute, encode, normalize, resample, synthesize, or calculate alloy descriptors yet. |
