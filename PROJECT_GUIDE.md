...
| `master_19papers_recovery_v13` | `data/processed/master_19papers_recovery_v13.csv` | 19 | 207 | Verified P002 source-specific recovery over immutable V12-QC: all 192 input rows and 343 input columns are cell-preserved; three exact annealing-defined replacements, ten correlated mechanism observations, two Hall-Petch support states, and field-level provenance are appended | CURRENT RECOVERY DATASET / V12 QC-SCHEMA-SPLITS REQUIRE REFRESH |
...
**Post-V13 status:** this section records the frozen V12 design and its counts; it is retained for reproducibility but is stale after P002 recovery V13. No candidate, manifest, M2 completeness statistic, or four-state count below may be used as current V13 evidence until Global QC, coverage/schema statistics, and grouped split feasibility are refreshed.
...
| ISS-005 | Small and imbalanced independent target classes. | Limits stable training, calibration, subgroup evaluation, and performance claims. | P1 | OPEN | V13 has 27/4 TRIP and 24/5 TWIP positive/negative conditions after the unsupported P002 A600 zeros become effective NA. V12 group-support and M2 figures are stale; refresh them before using any split. Expand diverse independent negatives without fabrication or resampling. |
| ISS-006 | Recovery V13 replacement-aware counting identifies 51 independent experimental ML conditions, with only 31 TRIP-labelled, 29 TWIP-labelled, and 26 jointly labelled. | Effective sample size remains small despite 207 observation/support rows. | P1 | OPEN | P002 exact rows replace three legacy primary representations without increasing independence. V12 split candidates cannot be reused as current; rerun Global QC, coverage/schema statistics, and grouped split design before matrix construction. |
| ISS-007 | V12 reported grain size as 56.12% missing; V13 adds scoped P002 A600/A700 RZ sizes and an A800 full average, but global coverage is not yet refreshed. | Important microstructure dependence may be omitted or selection-biased. | P2 | OPEN | Refresh V13 coverage; retain RZ/NRZ/full-average definition and method instead of merging scopes. |
| ISS-008 | V12 reported SFE as 75.51% missing and methods are heterogeneous; V13 recovers P002 approximately 14 mJ/m2 at 300 K only as a current-paper thermodynamic estimate. | Sparse/method-confounded phase-stability descriptor. | P2 | OPEN | Preserve estimate/method/temperature scope, do not call it experimental SFE, refresh coverage, and perform no empirical imputation. |
| ISS-009 | V12 reported DeltaG as 92.86% missing; V13 recovers P002 `-292 J/mol` at 300 K by Thermo-Calc TCFE7. | Phase-stability modelling is poorly supported. | P2 | OPEN | Refresh V13 coverage and retain sign, temperature, CALPHAD method, and alloy scope; never transfer across papers/alloys. |
| ISS-010 | V12 initial FCC/HCP coverage is stale. V13 establishes qualitative single FCC and direct HCP=0 for all three P002 exact conditions, while exact numeric FCC fractions remain NA. | Initial state can be confused with deformation-induced transformation. | P2 | OPEN | Refresh V13 coverage and preserve qualitative phase identity, direct absence, and numeric-fraction scope separately. |
...
| ISS-013 | Mechanical outcomes and post-deformation descriptors could create feature leakage. | Models could predict labels using consequences of the mechanism. | P1 | RESOLVED_ARCHITECTURALLY / V13 REFRESH REQUIRED | Feature Schema V1 freezes the pre-tensile prediction rule for 343 V12 fields. V13 adds source fields and explicitly classifies P002 mechanics and Hall-Petch fits as leakage-sensitive, but the schema inventory/coverage must be regenerated before use. |
| ISS-014 | No final target or final ML-ready dataset exists. | Model comparison/publication claims would be premature. | P1 | OPEN | P002 V13 changes usable targets and removes the former joint `00` observation. V12 Feature Schema coverage and Grouped Split Design V1 are retained as historical artifacts but are not current for V13. Refresh QC/coverage/schema/splits before any matrix; no model may yet be trained. |
...
| 2 | QC and provenance | IN PROGRESS: V12 global QC remains preserved, and P002 V13 adds field-level provenance; V13 Global QC and coverage refresh remain required. |
| 3 | Hierarchical experimental identity | IMPLEMENTED WITH CONSERVATIVE FALLBACKS: V13 remains 51 replacement-aware conditions; P002 adds one exact study series/material parent for three processing states, while unknown physical batches remain NA. The full V13 index must be regenerated. |
...
| 9 | Grouped/external validation | V12 SPLIT DESIGN V1 PRESERVED BUT STALE FOR V13: target counts changed and joint `00` support is now zero; rerun grouped feasibility before matrix construction. No model validation or performance result has occurred. |
...
|---|---|
| Current stage | P002 scientific evidence recovery V13 is complete. The V12 Global QC, Feature Schema V1 coverage, and Grouped Split Design V1 remain preserved historical artifacts but are stale for V13; matrix construction is no longer authorized until they are refreshed. No ML training, feature transformation, or performance evaluation has occurred. |
| Current canonical dataset | `data/interim/master_19papers_hierarchical_ids.csv` |
| Current recovery source dataset | `data/processed/master_19papers_recovery_v13.csv` (207 rows, 390 columns; all 192 V12-QC rows and all 343 V12-QC columns cell-preserved) |
| Current global-QC dataset | `data/processed/master_19papers_recovery_v12_qc.csv` remains the last completed QC artifact and immutable V13 input, but it is not a current QC assessment of the 207-row V13 dataset. |
| Current feature schema | Feature Schema V1 remains the frozen V12 prediction-time policy and classifies all 343 V12 fields. V13 has 390 columns, so its inventory and all coverage statistics must be refreshed without weakening the pre-tensile leakage rules. |
| Current grouped split design | Grouped Split Design V1 remains a reproducible V12 artifact only. V13 changes usable labels and removes the former joint `00` condition, so no V1 candidate or manifest may be treated as a current V13 split until grouped feasibility is regenerated. |
| Frozen prediction moment | Pre-deformation condition-level mechanism prediction, immediately before tensile loading begins. |
| Proposed feature-family progression | The V12 M1-M5 hierarchy and M2 baseline focus remain the intended policy, but candidate counts and complete-case support are stale after V13. No transformed matrix exists. |
| Number of papers | 19 |
| Number of rows | 207: 192 immutable V12-QC rows plus three P002 exact replacements, ten correlated P002 stage/post-test evidence records, and two non-independent Hall-Petch support states. |
| Latest independent-condition estimate | 51 replacement-aware experimental ML conditions; P002's three exact conditions replace three legacy primary representations. The twelve P017 computational conditions remain separate and unchanged. A refreshed V13 condition index is still required. |
| Current target status | Under review. V13 replacement-aware usable conditions are TRIP 31 (27/4), TWIP 29 (24/5), and joint 26 (`10=5`, `01=4`, `11=17`, `00=0`); 25 conditions have at least one unresolved component. P002 A600 is correctly `NA/NA`, not a negative. Group-level and M2-complete support are not yet refreshed. |
| Major unresolved issue | Twenty-five experimental conditions have incomplete targets; independent negatives are even scarcer; joint `00` has no current support; P018/P019 sources remain unavailable; measured chemistry, exact initial fractions, experimental SFE, and physical batches remain sparse. Paper/material dependence and all V12 coverage/split statistics block current matrix construction or confirmatory claims. |
| Next action | Run a non-destructive V13 Global QC refresh, then refresh feature coverage and Feature Schema coverage statistics, and finally redesign grouped T1/T2/T3 splits using the V13 replacement-aware roster. Only after those gates pass may an untransformed M2 condition table be considered. Do not train, impute, encode, normalize, resample, synthesize, reconcile chemistry, or calculate alloy descriptors yet. |
