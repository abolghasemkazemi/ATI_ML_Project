...
|---|---|
| `Paper_ID` | Stable project identifier for a source paper (currently P001–P020); the outer provenance and leave-one-paper-out grouping unit. New primary sources beyond the original P001–P019 set receive the next stable ID after DOI/source-identity and duplicate review. |
...
**Post-V14 status:** this section records the frozen V12 design and its counts; it is retained for reproducibility but is stale after P002 recovery V13 and the P020 V14 extension. No candidate, manifest, M2 completeness statistic, group-support result, or four-state count below may be used as current V14 evidence until Global QC, coverage/schema statistics, and grouped split feasibility are refreshed. V14 adds another exact `Fe50Mn30Co10Cr10` source family member in P020, but no frozen V12 split artifact has been edited to accommodate it.
...
| ISS-005 | Small and imbalanced independent target classes. | Limits stable training, calibration, subgroup evaluation, and performance claims. | P1 | OPEN | V14 has 28/4 TRIP and 25/5 TWIP positive/negative conditions. P020 adds a joint positive but no negative-family support. V12 group-support and M2 figures are stale; refresh them only after the current collection batch. Expand diverse independent negatives without fabrication or resampling. |
| ISS-006 | Extended recovery V14 replacement-aware counting identifies 52 independent experimental ML conditions, with only 32 TRIP-labelled, 30 TWIP-labelled, and 27 jointly labelled. | Effective sample size remains small despite 214 observation/support rows. | P1 | OPEN | P020 contributes one new independent condition; its six stages add no independence. V12 split candidates cannot be reused as current; rerun Global QC, coverage/schema statistics, and grouped split design before matrix construction. |
| ISS-007 | V12 reported grain size as 56.12% missing; V13 added scoped P002 sizes and V14 adds an approximately 40 um P020 FCC-average grain size, but global coverage is not yet refreshed. | Important microstructure dependence may be omitted or selection-biased. | P2 | OPEN | Refresh V14 coverage after collection; retain phase/RZ/NRZ/full-average definition and method instead of merging scopes. |
| ISS-008 | V12 reported SFE as 75.51% missing and methods are heterogeneous; V13 recovers P002 approximately 14 mJ/m2 at 300 K only as a thermodynamic estimate, while P020 remains NA. | Sparse/method-confounded phase-stability descriptor. | P2 | OPEN | Preserve estimate/method/temperature/paper scope, never import another Fe50Mn30Co10Cr10 value into P020, refresh coverage later, and perform no empirical imputation. |
| ISS-009 | V12 reported DeltaG as 92.86% missing; V13 recovers P002 `-292 J/mol` at 300 K by Thermo-Calc TCFE7, while P020 remains NA. | Phase-stability modelling is poorly supported. | P2 | OPEN | Refresh V14 coverage later and retain sign, temperature, CALPHAD method, alloy, and paper scope; never calculate or transfer P020 DeltaG. |
| ISS-010 | V12 initial FCC/HCP coverage is stale. V13 establishes qualitative single FCC/direct HCP=0 for three P002 conditions; V14 adds direct P020 initial FCC/HCP fractions 0.79/0.21 from EBSD and neutron diffraction. | Initial state can be confused with deformation-induced transformation. | P2 | OPEN | Refresh V14 coverage and preserve pre-existing HCP as initial state; P020 TRIP is supported by dynamic FCC loss, not by initial HCP presence. |
...
| ISS-013 | Mechanical outcomes and post-deformation descriptors could create feature leakage. | Models could predict labels using consequences of the mechanism. | P1 | RESOLVED_ARCHITECTURALLY / V14 REFRESH REQUIRED | Feature Schema V1 freezes the pre-tensile rule for 343 V12 fields. V14 has 454 columns and explicitly classifies P020 apparent yield onset, strength, elongation, phase evolution, and stage data as outcome/evidence leakage; inventory and coverage must be regenerated before use. |
| ISS-014 | No final target or final ML-ready dataset exists. | Model comparison/publication claims would be premature. | P1 | OPEN | P020 V14 raises joint `11` support but joint `00` remains absent. V12 Feature Schema coverage and Grouped Split Design V1 are historical only and do not include P002/P020 changes. Refresh QC/coverage/schema/splits after collection before any matrix; no model may yet be trained. |
...
|---:|---|---|
| 1 | Data extraction | IN PROGRESS: the original 19-paper extraction is preserved and P020 begins the verified extended-source recovery set. |
| 2 | QC and provenance | IN PROGRESS: V12 global QC remains preserved; P002 V13 and P020 V14 add field-level provenance. V14 Global QC and coverage refresh remain required after the current source-collection batch. |
| 3 | Hierarchical experimental identity | IMPLEMENTED WITH CONSERVATIVE FALLBACKS: V14 has 52 replacement-aware conditions. P020 adds one exact study/material condition and six correlated stages; unknown batch/replicate metadata remain NA. The full V14 index must be regenerated. |
| 4 | Target verification | IN PROGRESS: operational definitions and queue exist; source verification incomplete. |
| 5 | Targeted literature expansion | IN PROGRESS: P020 is the first new verified primary experimental paper beyond P001-P019; additional collection may continue before global refresh. |
...
| 9 | Grouped/external validation | V12 SPLIT DESIGN V1 PRESERVED BUT STALE FOR V14: P002/P020 changed target support, joint `00` remains zero, and P020 adds another Fe50Mn30Co10Cr10 source family. Rerun grouped feasibility after collection and before matrix construction. No model validation or performance result has occurred. |
...
|---|---|
| Current stage | P020 scientific evidence recovery V14 is complete as a new-source extension beyond P019. V12 Global QC, Feature Schema V1 coverage, and Grouped Split Design V1 remain preserved historical artifacts but are stale for V14. No ML training, feature transformation, or performance evaluation has occurred. |
| Current canonical dataset | `data/interim/master_19papers_hierarchical_ids.csv` |
| Current recovery source dataset | `data/processed/master_extended_recovery_v14.csv` (214 rows, 454 columns; all 207 V13 rows and all 390 V13 columns cell-preserved) |
| Current global-QC dataset | `data/processed/master_19papers_recovery_v12_qc.csv` remains the last completed QC artifact, but it does not assess P002 V13 or P020 V14 and is not current for the 214-row extended dataset. |
| Current feature schema | Feature Schema V1 remains the frozen V12 prediction-time policy and classifies 343 V12 fields. V14 has 454 columns; new phase-specific target/method/stage/outcome fields and all coverage statistics require a non-destructive refresh without weakening the pre-tensile rules. |
| Current grouped split design | Grouped Split Design V1 remains a reproducible V12 artifact only. V14 has changed usable support, lacks joint `00`, and adds P020 to the cross-paper Fe50Mn30Co10Cr10 family; no V1 candidate or manifest is a current V14 split. |
| Frozen prediction moment | Pre-deformation condition-level mechanism prediction, immediately before tensile loading begins. |
| Proposed feature-family progression | The V12 M1-M5 hierarchy and M2 baseline focus remain intended policy, but candidate counts and complete-case support are stale after V14. No transformed matrix exists. |
| Number of papers | 20 primary source IDs in the extended recovery dataset: original P001-P019 plus new P020. |
| Number of rows | 214: the immutable 207-row V13 prefix plus one P020 primary condition and six correlated P020 in-situ stage/end-point observations. |
| Latest independent-condition estimate | 52 replacement-aware experimental ML conditions. P020 contributes one; its stages contribute zero. The twelve P017 computational conditions remain separate and unchanged. A refreshed V14 condition index is required after collection. |
| Current target status | Under review. V14 usable conditions are TRIP 32 (28/4), TWIP 30 (25/5), and joint 27 (`10=5`, `01=4`, `11=18`, `00=0`). P020 is a verified HCP-TWIP `11`; it adds no negative support. Group-level and M2-complete support are not refreshed. |
| Major unresolved issue | Independent negative support remains only 4 TRIP and 5 TWIP, joint `00` remains absent, and 25 conditions still have at least one unresolved component. P020 supplement-defined test metadata, measured chemistry, SFE/DeltaG, physical batches, and many global descriptors remain sparse. Paper/material dependence and stale V12 QC/schema/split statistics block matrix construction or confirmatory claims. |
| Next action | Continue verified new-paper recovery if the collection batch is still open. When collection pauses, run a non-destructive V14 Global QC refresh, then refresh feature coverage/schema statistics and redesign grouped T1/T2/T3 splits with P020/common-family controls. Do not train, impute, encode, normalize, resample, synthesize, reconcile chemistry, or calculate alloy descriptors yet. |
